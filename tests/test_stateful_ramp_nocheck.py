...
class RampTest(NumbaGoUp):
    future_gamma = strategy(
        "uint256", min_value=int(2.8e-4 * 1e18 / 9), max_value=int(2.8e-4 * 1e18 * 9)
    )
    future_A = strategy(
        "uint256", min_value=90 * 2 ** 2 * 10000 // 9, max_value=90 * 2 ** 2 * 10000 * 9
    )

    def initialize(self, future_A, future_gamma):
        self.swap.ramp_A_gamma(
            future_A, future_gamma, self.chain.time() + 14 * 86400, {"from": self.accounts[0]}
        )
