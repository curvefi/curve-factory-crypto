...
@dev Assumed that metapool's 0-th coin is not ETH
"""
...
def exchange(_pool: address, i: uint256, j: uint256, _dx: uint256, _min_dy: uint256,
    ...
    if i < MAX_COIN:  # Swap to LP token and remove from base
        ...
        lp_amount: uint256 = CurveMeta(_pool).exchange(i, MAX_COIN, _dx, 0)
        ...
        if j == WETH_IDX and _use_eth:
            wETH(WETH).withdraw(coin_amount)
            raw_call(_receiver, b"", value=coin_amount)
        else:
    ...
    if j < MAX_COIN:
        ...
        return CurveMeta(_pool).exchange(MAX_COIN, j, lp_amount, _min_dy, False, _receiver)
...
def add_liquidity(
    ...
    for i in range(MAX_COIN, N_ALL_COINS):
        ...
        if i == WETH_IDX:
            if _use_eth:
                assert msg.value == amount  # dev: incorrect ETH amount
    ...
    return CurveMeta(_pool).add_liquidity(meta_amounts, _min_mint_amount, _use_eth, _receiver)
...
def remove_liquidity(
    ...
    for i in range(N_COINS):
        meta_received[i] = ERC20(CurveMeta(_pool).coins(i)).balanceOf(self)
    ...
    for i in range(MAX_COIN, N_ALL_COINS):
        ...
        if i == WETH_IDX and _use_eth:
            wETH(WETH).withdraw(amounts[i])
            raw_call(_receiver, b"", value=amounts[i])
        else:
...
def remove_liquidity_one_coin(
    ...
    coin_amount: uint256 = CurveMeta(_pool).remove_liquidity_one_coin(_burn_amount, MAX_COIN, 0, False, self)
