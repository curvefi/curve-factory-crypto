...
META_INDEX: constant(uint256) = META_N_COINS - 1
BASE_MAX_N_COINS: constant(uint256) = 4
ALL_N_COINS: constant(uint256) = META_N_COINS - 1 + BASE_MAX_N_COINS
...
@external
@view
def get_coins(_pool: address) -> address[ALL_N_COINS]:
    """
...
def _calc_price(_pool: address, _0_in_1: bool, _meta_price: uint256) -> uint256:
    ...
    # meta0 <--_meta_price-- meta1 --vp[1]--> base1
    if _0_in_1:
        return vprice * 10 ** 18 / _meta_price
    else:
        return _meta_price * 10 ** 18 / vprice
...
def lp_price(_pool: address, _i: uint256 = 0) -> uint256:
    ...
    vprice: uint256 = CurveBase(coin).get_virtual_price()
...
def exchange(_pool: address, i: uint256, j: uint256, _dx: uint256, _min_dy: uint256, _use_eth: bool = False, _receiver: address = msg.sender) -> uint256:
    ...
    i_pool: uint256 = min(i, META_INDEX)
    j_pool: uint256 = min(j, META_INDEX)
    pool: address = CurveMeta(_pool).coins(i_pool)
    coin: address = pool

    if i_pool == META_INDEX:
        adjusted_i: uint256 = i - META_INDEX
        coin = CurveBase(pool).coins(adjusted_i)
        eth_amount = self._receive(coin, amount, _use_eth, True)
        self._approve(coin, pool)

        if j_pool == META_INDEX:
            adjusted_j: uint256 = j - META_INDEX
            coin = CurveBase(pool).coins(adjusted_j)
            if not self._need_to_handle_eth(_use_eth, coin):
                receiver = _receiver

            amount = CurveBase(pool).exchange(convert(adjusted_i, int128), convert(adjusted_j, int128), amount, _min_dy, receiver, value=eth_amount)

            if receiver == self:
                return self._send(coin, _receiver, _use_eth)
            return amount

        amount = self._add_to_base_one(pool, amount, 0, adjusted_i, self, eth_amount)
        coin = pool
        eth_amount = 0
    else:
        eth_amount = self._receive(coin, amount, _use_eth, False)
    ...
    if j_pool == META_INDEX:
        adjusted_j: uint256 = j - META_INDEX
        pool = CurveMeta(_pool).coins(META_INDEX)
        coin = CurveBase(pool).coins(adjusted_j)
        self._approve(coin, pool)
        if not self._need_to_handle_eth(_use_eth, coin):
            receiver = _receiver
...
@internal
@view
def _calc_in_base_one(_pool: address, _amount: uint256, _i: uint256) -> uint256:
    """
...
def get_dy(_pool: address, i: uint256, j: uint256, _dx: uint256) -> uint256:
    ...
    if i_pool == META_INDEX:
        pool: address = CurveMeta(_pool).coins(META_INDEX)
        adjusted_i: uint256 = i - META_INDEX
        if j_pool == META_INDEX:
            adjusted_j: int128 = convert(j - META_INDEX, int128)
            return CurveBase(pool).get_dy(convert(adjusted_i, int128), adjusted_j, amount)
        amount = self._calc_in_base_one(pool, amount, adjusted_i)
    ...
    if j_pool == META_INDEX:
        pool: address = CurveMeta(_pool).coins(META_INDEX)
        adjusted_j: uint256 = j - META_INDEX
        amount = CurveBase(pool).calc_withdraw_one_coin(amount, convert(adjusted_j, int128))
...
@internal
def _add_to_base(_pool: address, _amounts: uint256[ALL_N_COINS], _use_eth: bool) -> uint256:
    """
...
def add_liquidity(
    _pool: address,
    _deposit_amounts: uint256[ALL_N_COINS],
    ...
    if _deposit_amounts[0] > 0:
        ...
        eth_amount += self._receive(coin, meta_amounts[0], _use_eth, False)
        self._approve(coin, _pool)

    for i in range(1, ALL_N_COINS):
        if _deposit_amounts[i] > 0:
            coin: address = CurveMeta(_pool).coins(1)
            meta_amounts[1] = self._add_to_base(coin, _deposit_amounts, _use_eth)
            self._approve(coin, _pool)
            break
...
@internal
@view
def _calc_in_base(_pool: address, _amounts: uint256[ALL_N_COINS]) -> uint256:
    """
...
@external
@view
def calc_token_amount(_pool: address, _amounts: uint256[ALL_N_COINS]) -> uint256:
    """
    ...
    """
    pool: address = CurveMeta(_pool).coins(1)
    meta_amounts: uint256[META_N_COINS] = [_amounts[0], 0]
    for i in range(1, ALL_N_COINS):
        if _amounts[i] > 0:
            coin: address = CurveMeta(_pool).coins(1)
            meta_amounts[1] = self._calc_in_base(coin, _amounts)
            break
...
@internal
def _remove_from_base(_pool: address, _min_amounts: uint256[ALL_N_COINS], _use_eth: bool, _receiver: address) -> uint256[ALL_N_COINS]:
    """
    ...
    returned: uint256[ALL_N_COINS] = empty(uint256[ALL_N_COINS])
    if n_coins == 2:
...
def remove_liquidity(
    ...
    _min_amounts: uint256[ALL_N_COINS],
    _use_eth: bool = False,
    _receiver: address = msg.sender,
) -> uint256[ALL_N_COINS]:
    """
    ...
    CurveMeta(_pool).remove_liquidity(_burn_amount, [_min_amounts[0], 0], _use_eth)

    returned: uint256[ALL_N_COINS] = empty(uint256[ALL_N_COINS])

    removed_from_base: bool = False
    coin: address = CurveMeta(_pool).coins(1)
    returned = self._remove_from_base(coin, _min_amounts, _use_eth, _receiver)

    coin = CurveMeta(_pool).coins(0)
    returned[0] = self._send(coin, _receiver, _use_eth)
