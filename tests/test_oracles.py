...
@given(i=strategy("uint8", min_value=0, max_value=1))
def test_price_scale_change(chain, crypto_swap_with_deposit, i, coins, accounts):
    ...
    step = max(
        crypto_swap_with_deposit.adjustment_step() / 1e18,
        abs(log(crypto_swap_with_deposit.price_oracle() / crypto_swap_with_deposit.price_scale()))
        / 5,
    )
    ...
    assert approx(
        crypto_swap_with_deposit.virtual_price(),
        crypto_swap_with_deposit.get_virtual_price(),
        1e-10,
    )
